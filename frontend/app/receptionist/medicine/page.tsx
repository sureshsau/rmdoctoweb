...
export default function ReceptionistMedicinePage() {
    ...
    // ...existing code for add/edit modal, form, and UI (can be copied from admin/medicine/page.tsx)

    // --- Begin full admin medicine UI, mobile responsive, with permission error handling ---
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!formData.dosageForm) { setFormError("Dosage form is required"); return; }
        if (selectedImages.length > 5) { setFormError("Maximum 5 images allowed"); return; }
        ...
        try {
            ...
            if (formData.manufacturer) basePayload.manufacturer = { name: formData.manufacturer };
            ...
            if (Object.keys(stockPayload).length > 0) basePayload.stock = stockPayload;
            if (editingId) {
    ...
    const imageCountLabel = `${selectedImages.length}/5`;
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-6">
                    {filteredMedicines.map((item) => (
                        <div key={item._id} className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden group hover:shadow-lg transition flex flex-col">
                            <div className="relative h-44 bg-linear-to-br from-gray-50 to-gray-100 flex items-center justify-center">
                                {(() => {
            ...
            {showAddModal && (
                <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4 backdrop-blur-sm">
                    <div className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl overflow-hidden flex flex-col max-h-[92vh] border border-gray-100">
                        <div className="p-4 border-b border-gray-100 flex justify-between items-center bg-linear-to-r from-cyan-50 to-white">
                            <div>
                                        ...
                                        <div className="border border-dashed border-cyan-200 rounded-xl bg-cyan-50/50 p-4">
                                            <div className="flex items-center justify-between">
                                                <div>
                                                    ...
                                                    <p className="text-[11px] text-gray-500">Up to 5 images. First one shows on cards.</p>
                                                </div>
                                            ...
                                            <label className="mt-3 flex flex-col items-center justify-center border border-dashed border-gray-300 rounded-lg py-6 bg-white hover:border-cyan-400 transition cursor-pointer">
