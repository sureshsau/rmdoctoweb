    ...
    <div className="max-w-2xl mx-auto p-4 sm:p-8">
      ...
      <div className="bg-slate-900 rounded-2xl p-6 mb-6 text-white">
        ...
        <div className="text-center mb-4">
          ...
          <div className="text-3xl font-extrabold mt-1">{wallet?.balance ?? user?.rmcredit ?? 0} RM</div>
        </div>
        ...
        <button className="w-full mt-2 flex items-center justify-center gap-2 bg-amber-100 text-amber-900 font-semibold py-2 rounded-xl hover:bg-amber-200 transition" onClick={() => { setTransferError(""); setTransferModal(true); }}>
      ...
      <form onSubmit={handleSearch} className="mb-4 flex gap-2">
        ...
        />
        <button type="submit" className="bg-amber-500 text-white px-4 py-2 rounded-lg hover:bg-amber-600 transition">
      ...
      <div className="space-y-3">
        ...
        {loadingMore && <div className="flex justify-center py-4"><Loader2 className="w-6 h-6 animate-spin text-amber-500" /></div>}
      </div>
      {/* Pagination Controls */}
      ...
      <Dialog open={transferModal} onClose={() => setTransferModal(false)} className="fixed z-50 inset-0 overflow-y-auto">
        <div className="flex items-center justify-center min-h-screen px-4">
          ...
          <div className="relative bg-white rounded-2xl p-6 w-full max-w-md mx-auto z-10">
            ...
            <button
              className="w-full bg-amber-500 text-white py-2 rounded-lg font-semibold hover:bg-amber-600 transition mb-2"
              onClick={handleTransfer}
              disabled={transferLoading}
            >
              {transferLoading ? <Loader2 className="w-5 h-5 animate-spin mx-auto" /> : "Transfer"}
            </button>
            <button className="w-full text-amber-500 py-2 rounded-lg font-semibold hover:underline" onClick={() => setTransferModal(false)}>
              Cancel
            </button>
          </div>
        </div>
